...
|---|---|---|---|
| P1 | Backup and restore conformance | M2.1 manifest contract, M2.2 state inspection, M2.3 backup creation and M2.4 restore preflight implemented locally | Supported PostgreSQL reconstruction fixture, state/history/provenance/audit comparison, credential non-persistence checks, and release verification. |
...
1. Begin M2.5 Database Reconstruction from the incremental sequence.
2. Keep recovery-mode integration and equivalence claims deferred until reconstruction is reviewed and complete.
