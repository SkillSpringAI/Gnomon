...
M2.1–M2.11 established a manifest and inspected backup, guarded reconstruction
into a pristine PostgreSQL target, canonical equivalence checks, entry into M1
...
Consolidate proven architectural boundaries while preserving behavior. This is a
bounded programme, not a rewrite or a mandate to split large files. **File size
alone is not a decomposition criterion.** Keep cohesive security-critical services
together where splitting them would make authority, locks, audit atomicity, or
recovery invariants harder to inspect.

### Ordered consolidation programme

1. **Documentation authority and invariant baseline.** Reconcile current truth,
   future work, normative rules, execution records, verification evidence, and
   history. Establish the compact invariant register and transaction vocabulary
   through C1 before extracting runtime boundaries.
...
4. **Investigation and execution service boundary.** Clarify ownership of task
   ...
   point-of-effect authority checks.
5. **Repository and read-projection boundary.** Separate durable write ownership
   from read projections where current responsibilities justify it. Retain one
   authoritative transaction owner per governed write and avoid repository layers
   created only for symmetry.
6. **Architectural invariant verification map.** Map each documented invariant to
   existing executable evidence, identify missing coverage honestly, and use that
   map to judge refactors. C1 establishes the baseline map; later consolidations
   must keep it current.

### Exit Gate

Each extracted boundary has a clear responsibility and transaction owner;
existing authority, history, recovery, and execution behavior remains equivalent
under the relevant regression and milestone verification. No split weakens a
security-critical invariant merely to reduce file size.

## Milestone 5: Shared Bounded Execution Vocabulary

### Objective

Describe source, provider, and agent execution through shared bounded vocabulary
and extract only narrow lifecycle primitives justified by existing paths. Keep
their orchestration and authority checks explicit before connecting live external
agents.

### Required Concepts

A shared description identifies, where applicable:

- the exact durable attempt and its task or operation scope;
- current authority and AuthorityEpoch binding where the effect requires it;
- deadline and budget;
- point-of-effect authority revalidation;
- outcome classification, including unknown external effects;
- interruption and cancellation limits;
- the reconciliation needed before a retry or authority restoration.

### Budget Hierarchy

Existing paths use different limits. Where work delegates or nests, make the
applicable parent and child bounds explicit for time, calls, requests, bytes,
retries, and governed mutations. A child cannot silently exceed its parent's
authority or budget. This does not require every execution kind to use one
common budget object.

### Non-goals

M5 does not require one generic `CycleRunner`, one generic workflow engine, or
erasure of source-, provider-, and agent-specific orchestration. A shared term
or narrow helper must not hide a distinct dispatch, transaction, authority,
interruption, or reconciliation contract.

### Exit Gate

Source retrieval, provider execution, and fake-agent execution use consistent
bounded terms where their semantics overlap, while their separate lifecycle and
authority contracts remain testable. Any extracted primitive preserves existing
attempt fencing, point-of-effect checks, unknown outcomes, and recovery behavior.
