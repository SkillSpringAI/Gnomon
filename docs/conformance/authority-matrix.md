...
| S10 | Fresh/continuing/recovery startup authority boundary | IMPLEMENTED | `application/authority_bootstrap.py`, `security_state_store.py`, `api/app.py`, migration 024 | `tests/integration/test_authority_bootstrap.py`, `tests/unit/test_security_state_store.py` | Recovery fence becomes effective before requests and has no clearing path; no RecoveryContext, protected restoration, backup import or epoch replacement |
