...
| S13 | Protected restoration preflight and completion | PARTIALLY_IMPLEMENTED | `domain/recovery.py`, `application/recovery_restoration_service.py` | `tests/integration/test_recovery_restoration_service.py` | One transaction consumes current RecoveryContext, M1.4 reconciliation, OperatorAuthorization and ExecutionAuthorization, then clears RECOVERY_REQUIRED with security-transition audit. It does not replace epochs, invalidate old authorizations or restore from backups. |
| S14 | Post-restoration authority epoch replacement | PARTIALLY_IMPLEMENTED | `domain/recovery.py`, `application/authority_epoch_replacement_service.py`, `application/authorization_service.py` | `tests/integration/test_authority_epoch_replacement_service.py`, `tests/integration/test_authorization_service.py` | Replacement requires restored NORMAL state and restoration audit evidence, rotates the singleton epoch, bumps version and appends audit. Old authorization records remain historical through exact replay; `require_current_execution` rejects old-epoch records and accepts new-epoch records for current protected effects. Exit-gate evidence and broader consumer audit remain open. |
