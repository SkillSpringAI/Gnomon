...
| A03 | Assessment and source-registry mutation audit; Slice 4 | IMPLEMENTED for trusted-source registration and activation | Configuration-scoped events are committed atomically with registration/activation, retain bounded actor/authority/status metadata, and are exposed behind READ_AUDIT | `tests/integration/test_source_registry_audit.py`, `src/research_agent/migrations/026_trusted_source_policy_audit.sql` | Provider-session lifecycle is tracked separately under A05; authenticated multi-user identity and unrelated configuration writers remain open |
...
| A05 | Authentication/security event audit; Slice 4 | PARTIALLY_IMPLEMENTED | Rejected memory commits and provider-session create/delete emit redacted events | `tests/integration/test_memory_rollback.py`, `test_provider_session_audit.py` | Add authenticated identity and unrelated provider/configuration-writer coverage |
...
| S09 | Direction-aware capability policy and authority administration | IMPLEMENTED | `src/research_agent/application/security_capability.py`, `security_transition_policy.py`, `security_state_service.py`, `source_registry.py` | `tests/unit/test_security_capability.py`, `test_security_transition_policy.py`; `tests/integration/test_security_state_transitions.py` | AUTHORITY_ADMINISTRATION governs state changes and trusted-source registration/activation; containment cannot broaden, recovery purpose cannot replace actual-effect authority; no OperatorAuthorization |
| S10 | Fresh/continuing/recovery startup authority boundary and RecoveryContext diagnostics | PARTIALLY_IMPLEMENTED | `application/authority_bootstrap.py`, `security_state_store.py`, `recovery_context_service.py`, `domain/recovery.py`, `persistence/recovery.py`, migrations 024/033 | `tests/integration/test_authority_bootstrap.py`, `tests/unit/test_security_state_store.py`, `tests/unit/test_recovery_context.py`, `tests/integration/test_recovery_context_service.py` | Recovery fence becomes effective before requests and M1.1 records bounded diagnostic RecoveryContext evidence. Protected restoration and epoch replacement now provide the local M1 clearing/lineage paths; backup import and reconstruction remain outside M1. |
| S11 | Operator and execution authorization artifacts for recovery authority | PARTIALLY_IMPLEMENTED | `domain/authorization.py`, `application/authorization_service.py`, `persistence/authorization.py`, migration 034 | `tests/unit/test_authorization_contract.py`, `tests/integration/test_authorization_service.py` | Local issuance/persistence, audit coupling, exact replay, current execution validation and closeout hardening are implemented. They do not authenticate multi-user identity or provide cryptographic tamper resistance. |
| S12 | Read-only recovery reconciliation verdict | PARTIALLY_IMPLEMENTED | `domain/recovery.py`, `application/recovery_reconciliation_service.py` | `tests/integration/test_recovery_reconciliation_service.py` | Supported evidence and provider/cycle operation streams produce deterministic passed/failed/unknown checks. The verdict is not persisted, does not repair state, does not consume authorization artifacts, and does not clear recovery bootstrap. |
| S13 | Protected restoration preflight and completion | PARTIALLY_IMPLEMENTED | `domain/recovery.py`, `application/recovery_restoration_service.py` | `tests/integration/test_recovery_restoration_service.py` | One transaction consumes current RecoveryContext, M1.4 reconciliation, OperatorAuthorization and ExecutionAuthorization, then clears RECOVERY_REQUIRED with security-transition audit. Epoch replacement is the separate S14 transition; backup reconstruction remains outside M1. |
| S14 | Post-restoration authority epoch replacement | PARTIALLY_IMPLEMENTED | `domain/recovery.py`, `application/authority_epoch_replacement_service.py`, `application/authorization_service.py` | `tests/integration/test_authority_epoch_replacement_service.py`, `tests/integration/test_authorization_service.py` | Replacement requires restored NORMAL state and restoration audit evidence, rotates the singleton epoch, bumps version and appends audit. Old authorization records remain historical through exact replay; `require_current_execution` rejects old-epoch records and accepts new-epoch records for current protected effects. Pass-3 local consumer audit and evidence mapping are complete, and Quality run `35981852013` passed for closeout SHA `316c90b`; M2 backup/reconstruction remains open. |
...
| AUTH-01-14 | §14 Auditability | PARTIALLY_IMPLEMENTED | A01,A02,A03,A04,A05 | Core mutation, rejected-memory security, and trusted-source registration/activation events are covered; provider-session, authenticated identity, and unrelated writer coverage remain open. Slice 4. |
