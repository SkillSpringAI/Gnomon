...
| B03 | Numbered transactional migration runner; Phase 0 | IMPLEMENTED | `src/research_agent/application/migrations.py` | `tests/unit/test_migrations.py`; integration suites invoke migrations | Checksum/downgrade handling deferred |
...
| B24 | Bounded provider resource consumption; Slice 5 | PARTIALLY_IMPLEMENTED | `src/research_agent/api/routes/reports.py`, Bedrock adapters | `tests/integration/test_reports.py::test_report_draft_budget_is_enforced` | Success count is not atomic reservation; no shared end-to-end deadline across retries; Slice 5 |
...
| A06 | Preserve history rather than silently destroy it; target milestone | CONFLICTING | `assessment_service.py` replaces current data/links; migration 001 cascades task deletion to events | Snapshot tests cover replacement, not historical preservation | Violates 03 §53, 04 §9/62, 06 §50/64; Slices 2–4 |
...
| AUTH-01-15 | §15 No Silent Loss of Knowledge | PARTIALLY_IMPLEMENTED | A06,B19,V01,V08 | Governed assessment changes retain journal history; task deletion still cascades audit history and dependent propagation is incomplete. Slice 4/recovery review. |
