...
The browser suite covers exact objective/URL payloads, successful and partial collection, busy controls, invalid URLs, stale-state recovery, reload, unavailable reports, narrow-screen layout, objective review, interrupted-cycle recovery, source-dependence history and reversal, stable operation-ID retries, cycle-qualified stopping references, and bounded graph overflow language. Tests are skipped unless explicitly enabled locally. When `RUN_BROWSER_TESTS=1` is set, missing Playwright is a hard failure rather than a skip, and the selected browser must launch successfully.
